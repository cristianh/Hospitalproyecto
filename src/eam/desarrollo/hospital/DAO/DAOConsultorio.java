...
public class DAOConsultorio implements IntConsultorio{
	...
	public void crear(Consultorio consultorio) throws Exception {
		...
		try {
			...
			String sql = "INSERT INTO  consultorio (id_consultario,descripcion_consultorio,id_estado_consultorio) VALUES (?,?,?)";
			...
			JOptionPane.showMessageDialog(null, "Consultorio registrado", "Info",
	...
	@Override
	public Consultorio buscar(String identificacion) throws Exception {
		Connection con = Conexion.getConexion();
		//String sql = "SELECT * from paciente WHERE numero_documento_paciente=?";
		String sql = "SELECT  c.id_consultario,c.descripcion_consultorio,c.id_estado_consultorio,es.id_estado_consultorio,es.descripcion_estado_consultorio from consultorio as c join estado_consultorio as es on c.id_estado_consultorio = es.id_estado_consultorio where c.id_consultario = ?";
		//String sql2 = "SELECT tipo_documento from tipodocumento WHERE id_documento=?";
	...
	public void eliminar(String id) throws Exception {
		...
		try {
			...
			// System.out.println(sql);
			Connection con = Conexion.getConexion();
			String sql2 = "DELETE from consultorio WHERE id_consultario=?";
...
try {
			...
			String sql = "UPDATE consultorio SET descripcion_consultorio=?,id_estado_consultorio=? where id_consultario=?";
			// System.out.println(sql);
	...
	public ArrayList<EstadoConsultorio> listarEstadoConsul(){
        ...
        //Tipodocumento tipDocumento;
		  try {
	    		...
	    		/*if(rs != null){
	    			tipodocumento=(Tipodocumento) rs;
	    		}*/
	            while (rs.next()) {
	            	estado= new  EstadoConsultorio(rs.getString("id_estado_consultorio"), rs.getString("descripcion_estado_consultorio"));
